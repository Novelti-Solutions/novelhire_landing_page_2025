...
const Categories = [
  {
    ...
    name: "General",
    icon: (isActive) => (
      <User className={cn(isActive ? "text-theme-primary" : "")} />
    ),
  },
  {
    id: 15,
    name: "Companies",
    icon: (isActive) => (
      <Building className={cn(isActive ? "text-theme-primary" : "")} />
    ),
  },
  {
    id: 16,
    name: "Recruiters",
    icon: (isActive) => (
      <Briefcase className={cn(isActive ? "text-theme-primary" : "")} />
    ),
  },
  {
    id: 17,
    name: "Colleges",
    icon: (isActive) => (
      <School className={cn(isActive ? "text-theme-primary" : "")} />
    ),
  },
];
  ...
  return (
    <div className="mx-auto max-w-screen-md">
      <div className="flex flex-col items-center gap-4">
        ...
        <h3 className="text-xl font-semibold md:text-3xl lg:text-5xl">
          Frequently Asked Questions
        </h3>
        <p className="text-sm text-muted-foreground">
          All Plans come with a{" "}
          <span className="text-sm font-bold text-black">14-day</span> free
          trial period.
        </p>
      </div>
        ...
        {Categories.map((category) => {
          return (
            <Button
              ...
              className={cn(
                "w-5/12 justify-start py-6 text-xs md:text-sm lg:text-base",
                category.id === currentCategory.id ? "bg-gray-100" : ""
              )}
              key={category.id}
            >
              {category.icon(category.id === currentCategory.id)}{" "}
              {category.name}
            </Button>
          );
        })}
      ...
      <div className="my-4 px-8">
        <h3 className="px-5 text-sm font-semibold md:text-lg lg:text-xl">
